...
# Allineamento G. Balance Zones Pro v0.5.3.6:
# - il ruolo operativo corrente usa la posizione della Structural rispetto al prezzo;
# - la dominanza storica S/R resta separata e continua a usare SOLO le statistiche della Balance.
# AREA ATTIVA e relative regole di screening restano invariate.
ROLE_SUPPORT = 1
ROLE_RESISTANCE = -1
ROLE_BALANCE = 0
...
def analyze_balance_zones(
    ...
    """Porting del Balance Zones Pro v0.5.3.6: A Structural + B Reaction.

    A usa Validation 10; B è una seconda istanza indipendente con Validation 5.
    I motori A/B restano invariati rispetto alla v0.5.3.4; max Balance = 9.
...
def plot_balance(data: pd.DataFrame, ticker: str, active_center: float, active_bottom: float, active_top: float, dominance_label: str) -> go.Figure:
    """Grafico AREA ATTIVA allineato alla lettura operativa v0.5.3.6.

    Structural: sotto il prezzo = Supporto, sopra = Resistenza, dentro = IN.
    Reaction B resta neutrale/grigia. La dominanza storica S/R resta separata
    nella tabella e nei relativi filtri, senza modificare AREA ATTIVA.
    """
    chart = data.tail(220)
    ...
    hist_role = str(dominance_label).upper()
    last_close = float(data.iloc[-1]["close"])
    if hist_role == "REACTION":
        current_role = "REACTION"
    ...
    fig.add_hrect(
        ...
        line_width=0,
        annotation_text=f"AREA ATTIVA · {current_role}",
        annotation_position="top left",
    )
    fig.add_hline(y=float(active_center), line_width=3, line_color=line_color)

    fig.update_layout(
        title=f"{ticker} · Daily chiusa · AREA ATTIVA · {current_role}",
        height=650,
